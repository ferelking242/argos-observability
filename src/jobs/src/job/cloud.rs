...
async fn report_org_no_ingestion(start_hour: i64, duration: &str) {
    ...
    for org in orgs {
        ...
        if streams.is_empty() {
            report_to_segment(
                "Argos Observability - No ingestion after creation",
                &org.identifier,
                duration,
            )
            .await;
    ...
    loop {
        ...
        for (org, _) in org_cache {
            ...
            if !orgs.contains(&org) && trial_period {
                report_to_segment("Argos Observability - No ingestion in last day", &org, "last 1 day")
                    .await;
            }
        }
        ...
        for (org, _) in org_cache {
            ...
            if subscription.is_none_or(|s| s.subscription_type.is_free_sub()) {
                ...
                if now <= org_record.trial_ends_at {
                    ...
                    report_to_segment(
                        "Argos Observability - Org expiry reminder",
                        &org,
...
async fn check_external_contract_expiry() {
    ...
    for cb in billings {
        ...
        report_to_segment(
            "Argos Observability - Contract expiry reminder",
            org_id,
