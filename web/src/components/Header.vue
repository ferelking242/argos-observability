...
<template>
  <div class="flex flex-nowrap items-center h-10 w-full bg-surface-chrome-deeper shrink-0">
    ...
    <!-- LOGO SECTION: Displays custom or default Argos Observability logo -->
    ...
    >
      ...
      <!-- Argos logo (shown alongside custom logo if configured) -->
      <div
        ...
        <a :href="homeUrl" @click.prevent="goToHome" class="inline-flex items-center">
          <img
            ...
            :src="
              getImageURL(
                store.state.theme === 'dark'
                  ? 'images/common/argos_logo_dark.svg'
                  : 'images/common/argos_logo.svg',
              )
            "
            alt="Argos Observability"
          />
        </a>
      </div>
    </div>

    <!-- Default Argos logo (when no custom logo) -->
