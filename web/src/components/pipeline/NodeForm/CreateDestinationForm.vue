      ...
      >
        ...
        >
          ...
          <div class="flex flex-col gap-4">
            ...
            <!-- Argos Observability Organization and Stream fields -->
            <div
            ...
            >
              <div class="w-1/2">
                <OFormInput
                  ...
                  help-text="Argos Observability organization identifier"
                  tabindex="0"
                />
              </div>
              <div class="w-1/2">
                <OFormInput
                  ...
                  required
                  :placeholder="'e.g., default'"
                  help-text="Argos Observability stream name"
                  tabindex="0"
                />
              </div>
            </div>
...
// when a non-Argos type is selected, and shown as a hint).
...
// Build the create-mode default record. Argos is the default type, so seed
// the Argos Observability endpoint + default headers.
const buildCreateDefaults = (): DestinationForm => ({
  ...
  url_endpoint: "/api/default/default/_json", // Default endpoint for Argos
  method: "post",
  output_format: "json",
  esbulk_index: "",
  separator: "",
...
watch(
  ...
  (newType, prev) => {
    ...
    if (newType === "openobserve") {
      // For Argos, use org and stream values.
      form.setFieldValue(
        "url_endpoint",
...
const populateFormForEdit = (destination: any) => {
  ...
  // Extract Argos org and stream from endpoint if it's Argos
...
// Keep the Argos Observability endpoint in sync with org/stream as the user edits them.
...
const connectionNotes = computed(() => {
  switch (destinationType.value) {
    case "openobserve":
      return {
        title: "Argos Observability Connection Details",
        steps: [
          "Log in to your Argos Observability instance",
          ...
          "Add authentication header: Authorization: Basic <Argos_Token>",
        ],
        example:
...
// Reset the form back to the create-mode Argos defaults (single source of
