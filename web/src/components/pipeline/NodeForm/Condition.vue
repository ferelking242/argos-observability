...
const updateStreamFields = async (streamName: any, streamType: any) => {
  ...
  if (
    ...
    // These are Argos Observability internal fields that should always be available
    const timestampColumn =
      store.state.zoConfig?.timestamp_column || "_timestamp";
    const allFieldsName = store.state.zoConfig?.all_fields_name;

    // Filter the columns to include:
    // 1. System fields (timestamp and all_fields) - always needed for Argos Observability functionality
