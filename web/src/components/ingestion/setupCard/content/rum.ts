...
const cdnInstall = (subs: RumCardSubs) => `<!-- Performance: resolve DNS + open the TLS connection early for the CDN and
     your Argos Observability endpoint, so neither is on the SDK's critical path. -->
...
export default function rumCard(subs: RumCardSubs): RichCardContent {
  ...
  return {
    ...
    extras: {
      ...
      fixBody:
        "If your app runs but nothing arrives, the browser most likely blocked the SDK or its requests — usually a Content-Security-Policy or an ad-blocker. Allow the CDN and your Argos Observability endpoint, then reload:",
      fixLang: "yaml",
  ...
  worker-src 'self' blob:;`,
      troubleshooting: [
        ...
        {
          q: "My Argos Observability endpoint is plain HTTP",
    ...
    docUrl: "https://github.com/argos-observability/argos-observability/wiki/user-guide/data-exploration/rum/setup/",
  };
}
