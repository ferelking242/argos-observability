...
<!--
  Copy/paste setup for connecting an MCP client to Argos Observability's INBOUND
...
http_headers = { "Authorization" = "${auth}" }`
  ...
  },
  {
    ...
    deepLink: (ep, auth) =>
      auth
        ? null
        : `https://claude.ai/settings/connectors?modal=add-custom-connector&mcpName=ArgosObservability&mcpServerUrl=${encodeURIComponent(ep)}`,
  },
