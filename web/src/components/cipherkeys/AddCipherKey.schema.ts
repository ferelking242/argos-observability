...
// the Argos `local` secret, and the simple-algorithm requirement) live in
...
export const makeAddCipherKeySchema = (t: (_key: string) => string) =>
  ...
  .object({
    ...
    key: z.object({
      store: z.object({
        // Store type select (Argos `local` vs `akeyless`) — required.
        ...
        // Argos secret (AddArgosType's only field). Required only
  ...
  .superRefine((val, ctx) => {
    ...
    // ── Argos (local) store → secret required ────────────────────────
    if (storeType === "local") {
